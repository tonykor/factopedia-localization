...
return [
    ...
    'Too many objects were selected' => 'Слишком много объектов было выбрано',
    ...
    'Move to root' => 'Перенести в корень',
    ...
    'Please {register} or {login} to get access' => 'Пожалуйста {register} или {login} что бы получить доступ',
    'login' => 'войти',
    ...
    'Showing results {from} - {to} of {total}' => 'Показаны результаты {from} - {to} из {total}',
    'Nothing was found' => 'Ничего не найдено',
    'No values' => 'Нет значений',
    'Go to' => 'Перейти к',
    'Select property' => 'Выберите свойство',
    '[A]dd' => '[Д]обавить',
    'You can group by up to {max} attributes' => 'Вы можете групировать максимально до {max} атрибутов',
    'It\'s the maximum amount of attributes you can group by' => 'Это максимальное кол-во атрибутов для группировки',
    'You can go to up to {max} values' => 'Вы можете перейти только к {max} значениям',
    '{objects} starting from letter `{chr}`' => '{objects} начинающиеся в буквы `{chr}`',
    '{objects} grouped by {groupBy}' => '{objects} сгрупированны по {groupBy}',
    '{objects} where `{attribute}`=`{value}`' => '{objects} где `{attribute}`=`{value}`',
    //versions
    ...
    '{name}{parentName} was marked for {status}, new version {version}' => '{name}{parentName} был помечен на {status}, новая версия {version}',
